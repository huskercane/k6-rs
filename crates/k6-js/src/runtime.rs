...
// ---------------------------------------------------------------------------
// The per-VU QuickJS runtime. Each coroutine VU (`coroutine_vu`) owns one — a
// synchronous `Runtime`/`Context` wrapped in a corosensei coroutine so blocking
// host fns YIELD instead of `block_on`. (The async `AsyncRuntime`/`AsyncContext`
// substrate explored in the Phase-0 spike is NOT the VU path — the B2 coroutine
// model superseded it; see ASYNC_RUNTIME_PLAN.md.)
// ---------------------------------------------------------------------------

/// Create a new QuickJS runtime with the per-VU limits.
///
/// Each VU gets its own runtime for full isolation (no GC pauses crossing VUs) and
/// its own 64 MB heap cap.
